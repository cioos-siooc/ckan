...
COPY ./contrib/docker/src/ckanext-hierarchy $CKAN_VENV/src/ckanext-hierarchy
...
RUN /bin/bash -c "source $CKAN_VENV/bin/activate && cd $CKAN_VENV/src && ckan-pip install -r ckanext-hierarchy/requirements.txt"
RUN /bin/bash -c "source $CKAN_VENV/bin/activate && cd $CKAN_VENV/src/ckanext-hierarchy && python setup.py install && python setup.py develop"
