...
RUN ckan-pip install configparser
...
COPY ./contrib/docker/src/pygeometa $CKAN_VENV/src/pygeometa
...
RUN /bin/bash -c "source $CKAN_VENV/bin/activate && cd $CKAN_VENV/src && ckan-pip install -r pygeometa/requirements.txt"
RUN /bin/bash -c "source $CKAN_VENV/bin/activate && cd $CKAN_VENV/src/pygeometa && python setup.py install && python setup.py develop"
